...
import { CanonicalGaussianPlyLoader } from "../../renderer/CanonicalGaussianPlyLoader";
...
} from "../../renderer/store-attributes/GaussianStoreAttributes";
import {
  type GaussianStoreAttributePacker,
  type GaussianStoreAttributeUploadStats,
  type PackedLayoutCell,
} from "../../renderer/store-attributes/GaussianStoreAttributePacker";
import { PackedLodLevelAttributePacker } from "../../renderer/store-attributes/PackedLodLevelAttributePacker";
import type { GaussianStorePackedAttribute } from "../../renderer/store-attributes";
import {
  markSlotRangesUpdated,
  mergeSlotRanges,
  rangeSlotCount,
} from "../../renderer/utils/slotRanges";
 ...
 */
export class LocalGaussianBackend implements GaussianBackend {
  ...
  async loadBuffer(
    ...
    try {
      ...
      lod = GaussianLod.build(octree, { ...options.lod, ownsOctree: true });
  ...
  getSourceCount(cloud: GaussianCloud): number {
    const entry = this.entries.find((item) => item.cloud === cloud);
    if (entry === undefined)
      throw new Error("Cloud does not belong to this Store");
    return entry.sourceGaussianCount;
  }
  ...
  getBounds(
    ...
    if (entry === undefined)
      throw new Error("Cloud does not belong to this Store");
    if (entry.lod !== null) {
      const { min, max } = entry.lod.octree.bounds;
      return [min.x, min.y, min.z, max.x, max.y, max.z];
    }
    const values = entry.source!.means.array as Float32Array;
    const bounds = [
      Infinity,
      Infinity,
      Infinity,
    ...
    for (let index = 0; index < entry.sourceGaussianCount; index++) {
      for (let axis = 0; axis < 3; axis++) {
        const value = values[index * 4 + axis]!;
  ...
  pack({ limits }: GaussianStorePackOptions): void {
    ...
    for (const listener of this.changeListeners)
      listener({
        type: "changed",
        reason: canUpdateInPlace ? "content" : "layout",
      });
  }
  ...
  packLodBatch(cloud: GaussianCloud): GaussianStoreLodBatchResult {
    ...
    this.packedContentVersion++;
    for (const listener of this.changeListeners)
      listener({ type: "changed", reason: "content" });
  ...
  invalidateCloudPacking(cloud: GaussianCloud): void {
    ...
    for (const listener of this.changeListeners)
      listener({ type: "changed", reason: "layout" });
  }
  ...
  private invalidatePacking(): void {
    ...
    for (const listener of this.changeListeners)
      listener({ type: "changed", reason: "clouds" });
  }
...
function createDefaultPackingStrategy(
  options: GaussianStoreDefaultLodOptions,
): StreamingLodPackingStrategy<TieredRadialLodPackingStrategy> {
