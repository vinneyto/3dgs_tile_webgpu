...
export class StreamingGaussianBackend implements GaussianBackend {
  ...
  constructor(config: BackendConfig) {
    if (config.frontend.maxStorageBufferBindingSize <= 0 || config.frontend.maxBufferSize <= 0)
  ...
  private async handle(command: Exclude<BackendCommand, { type: "cancel" }>): Promise<void> {
    switch (command.type) {
      case "load-cloud":
      case "load-cloud-from-buffer": {
        ...
        try {
          if (command.type === "load-cloud") {
            ...
            source = this.parser.parse(await response.arrayBuffer());
          } else {
        ...
        for (const attribute of options.attributes ?? []) {
          if (CORE_ATTRIBUTES.has(attribute.name) || entry.attributes.has(attribute.name))
            throw new Error(`Reserved or duplicate attribute name: ${attribute.name}`);
          entry.attributes.set(attribute.name, createAttribute(attribute, source.count));
        }
        for (const other of this.clouds.values())
          for (const [name, attribute] of entry.attributes) {
            ...
            if (existing && (existing.format !== attribute.format ||
              existing.elementsPerGaussian !== attribute.elementsPerGaussian))
              throw new Error(`Attribute schema differs across clouds: ${name}`);
          }
        ...
        this.emit({
          ...
          raycast: entry.raycastable ? createRaycastSnapshot(octree) : undefined,
        });
        this.target = null;
        this.replace(packed);
        return;
      ...
      case "unload-cloud":
        ...
        this.emit({ type: "cloud-unloaded", commandId: command.id, cloudId: command.cloudId });
        this.repack();
        return;
      case "set-cloud-priority":
        this.getCloud(command.cloudId).priority = validatePriority(command.priority);
        this.repack();
        break;
      case "set-cloud-packing":
        this.getCloud(command.cloudId).packingStrategy = command.packingStrategy;
        this.repack();
        break;
      ...
      case "set-cloud-raycastable": {
        ...
        this.emit({
          ...
          raycast: entry.raycastable ? createRaycastSnapshot(entry.octree) : undefined,
        });
        return;
      }
  ...
  private writeRange(command: Extract<BackendCommand, { type: "write-attribute-range" }>): void {
    const entry = this.getCloud(command.cloudId);
    const { firstGaussian: start, gaussianCount: count, attribute: name } = command;
    if (!Number.isSafeInteger(start) || !Number.isSafeInteger(count) ||
      start < 0 || count < 0 || start + count > entry.source.count)
      throw new RangeError("Attribute range exceeds source cloud");
    if (name === "lodLevel") throw new Error("lodLevel is computed by the backend");
    let target: Values;
    let elements: number;
    switch (name) {
      case "means": target = entry.source.means.array; elements = 4; break;
      case "scalesOpacity": target = entry.source.scalesOpacity.array; elements = 4; break;
      case "rotations": target = entry.source.rotations.array; elements = 4; break;
      case "shCoefficients":
    ...
    const values = target instanceof Uint32Array
      ? new Uint32Array(command.data) : new Float32Array(command.data);
  ...
  private maxSlots(degree: number, extra: Map<string, AttributeValues>): number {
    const limit = Math.min(
      this.config.frontend.maxStorageBufferBindingSize,
      this.config.frontend.maxBufferSize,
    );
    const widths = [16, 16, 16, (degree + 1) ** 2 * 4, 4,
      ...[...extra.values()].map((value) => value.elementsPerGaussian * 4)];
    const supported = Math.min(...widths.map((width) => Math.floor(limit / width)));
    if (supported < 1) throw new RangeError("Frontend buffer limits are too small");
    return Math.min(supported, this.config.maxGaussians === "auto" ||
      this.config.maxGaussians === undefined ? supported : this.config.maxGaussians);
  }

  private compute(reserveCapacity = 0): PackedState {
    const ordered = [...this.clouds.values()].sort((a, b) =>
      a.priority - b.priority || a.objectId - b.objectId);
    const degree = ordered.reduce<0 | 1 | 2 | 3>(
      (result, entry) => Math.max(result, entry.source.shDegree) as 0 | 1 | 2 | 3, 0);
    ...
    const selections: {
      entry: CloudEntry; indices: Uint32Array; levels: Uint32Array; cellEnds: number[];
    }[] = [];
    for (const entry of ordered) {
      const packing = this.select(entry, Math.min(remaining, entry.source.count));
    ...
    const count = selections.reduce((sum, selection) => sum + selection.indices.length, 0);
    const capacity = Math.max(1, count, reserveCapacity);
    const attributes = new Map<string, AttributeValues>();
    const allocate = (name: string, format: "f32" | "u32", elementsPerGaussian: number) => {
      const values = format === "f32"
    ...
    for (const { entry, indices, levels, cellEnds } of selections) {
      ...
      for (let i = 0; i < indices.length; i++, slot++) {
        ...
        (means as Float32Array).set(source.means.array.subarray(index * 4, index * 4 + 4), slot * 4);
        means[slot * 4 + 3] = entry.objectId;
        (scalesOpacity as Float32Array).set(
          source.scalesOpacity.array.subarray(index * 4, index * 4 + 4), slot * 4);
        (rotations as Float32Array).set(
          source.rotations.array.subarray(index * 4, index * 4 + 4), slot * 4);
        lodLevel[slot] = levels[i]!;
        for (let coefficient = 0; coefficient < source.shCoefficientCount; coefficient++) {
          const base = (index * source.shCoefficientCount + coefficient) * 4;
          sh[slot * (degree + 1) ** 2 + coefficient] = packShRgb8e8(
            shValues[base]!, shValues[base + 1]!, shValues[base + 2]!);
        }
        for (const [name, value] of entry.attributes) {
          const dest = attributes.get(name)!.values;
          const size = value.elementsPerGaussian;
          dest.set(value.values.subarray(index * size, (index + 1) * size), slot * size);
        }
      }
      nextCellId += cellEnds.length;
      states.push({ cloudId: entry.id, objectId: entry.objectId, renderedCount: indices.length });
    }
  ...
  private select(entry: CloudEntry, budget: number): GaussianLodPacking {
    const center = this.cameraPosition.clone()
      .applyMatrix4(entry.transform.clone().invert());
    const config = entry.packingStrategy;
    switch (config.type) {
      case "maximum":
        return new MaximumLodPackingStrategy().pack({ lod: entry.lod, maxGaussians: budget });
      case "radial":
        return new RadialLodPackingStrategy({ center, lodLevel: config.lodLevel })
          .pack({ lod: entry.lod, maxGaussians: budget });
      case "tiered-radial":
        return new TieredRadialLodPackingStrategy({ center, budgetShares: config.budgetShares })
          .pack({ lod: entry.lod, maxGaussians: budget });
      case "distance-aware-radial":
        return new DistanceAwareRadialLodPackingStrategy({ center, levelDistance: config.levelDistance })
          .pack({ lod: entry.lod, maxGaussians: budget });
    }
  }
  ...
  private updateTarget(): void {
    ...
    if (desired.capacity !== this.packed.capacity ||
      desired.degree !== this.packed.degree ||
      [...desired.attributes].some(([name, descriptor]) =>
        this.packed?.attributes.get(name)?.elementsPerGaussian !== descriptor.elementsPerGaussian)) {
  ...
  private replace(packed: PackedState): void {
    ...
    this.layoutVersion++;
    this.contentVersion++;
    const attributes: PackedAttributeBuffer[] = [...packed.attributes].map(([name, attribute]) => ({
      name, format: attribute.format,
  ...
  private emitNextPatch(): void {
    ...
    const maxBytes = this.config.streamingLod?.maxUploadBytesPerUpdate ?? 1024 * 1024;
    const maxCells = Math.max(1, this.config.streamingLod?.maxChangedCellsPerUpdate ?? 16);
    const bytesPerSlot = [...current.attributes.values()]
      .reduce((sum, attribute) => sum + attribute.elementsPerGaussian * 4, 0);
    const batchSlots = Math.max(1, Math.floor(maxBytes / bytesPerSlot));
    const changed: number[] = [];
    const changedCells = new Set<number>();
    for (let slot = 0; slot < current.capacity && changed.length < batchSlots; slot++) {
      if ([...current.attributes].some(([name, attribute]) => {
    ...
    if (changed.length === 0) {
      if (JSON.stringify(current.clouds) !== JSON.stringify(target.clouds)) {
        ...
        this.emit({
          type: "buffers-patched", sceneRevision: this.sceneRevision,
          layoutVersion: this.layoutVersion, baseContentVersion,
          contentVersion: this.contentVersion, patches: [],
          changedClouds: target.clouds, lodPending: false,
        });
      }
      this.packed = { ...current, count: target.count, clouds: target.clouds, cells: target.cells };
      this.target = null;
      return;
    }
    for (const [name, attribute] of current.attributes) {
      ...
      const flush = () => {
        ...
        patches.push({ name, firstSlot: first, slotCount: previous - first + 1,
...
function createAttribute(input: AttributeInit, count: number): AttributeValues {
  ...
  const values = input.format === "f32"
    ? new Float32Array(length) : new Uint32Array(length);
  if (input.source.kind === "fill")
    ...
    values.set(input.format === "f32"
      ? new Float32Array(input.source.data) : new Uint32Array(input.source.data));
  }
