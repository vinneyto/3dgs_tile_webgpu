...
describe("message backend", () => {
  it("resolves relative PLY URLs on the client before loading", async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith("http://localhost:5173/sandbox/mug.ply",
  ...
  it("transfers source data, keeps full raycast on client, and patches custom attributes", async () => {
    ...
    const cloud = await store.loadBuffer(input, {
      attributes: [{
        name: "weight", format: "u32", elementsPerGaussian: 1,
    ...
    expect((store.getPackedAttribute("weight")!.array as Uint32Array)[0]).toBe(1);
    const raycast = () => {
      const hits: ReturnType<Raycaster["intersectObject"]> = [];
      cloud.raycast(new Raycaster(new Vector3(0, 0, 2),
        new Vector3(0, 0, -1)), hits);
      return hits;
    };
    ...
    await vi.waitFor(() =>
      expect((store.getPackedAttribute("weight")!.array as Uint32Array)[0]).toBe(7));
    ...
    expect(port.events.some((event) => event.type === "buffers-patched")).toBe(true);
    store.dispose();
  ...
  it("cancels an in-flight URL load and never publishes its cloud", async () => {
    ...
    vi.stubGlobal("fetch", fetcher);
    backend.dispatch({ type: "load-cloud", id: "load", cloudId: "cloud", url: "https://example.test/a.ply" });
    await vi.waitFor(() => expect(fetcher).toHaveBeenCalled());
    backend.dispatch({ type: "cancel", id: "cancel", targetCommandId: "load" });
    await vi.waitFor(() =>
      expect(events).toContainEqual({ type: "command-cancelled", commandId: "load" }));
  ...
  it("packs several clouds, fills missing attributes and sends only changed ranges", async () => {
    ...
    const store = new GaussianStore(new WorkerStreamingGaussianBackend(
      { ...DEFAULT_BACKEND_CONFIG, maxGaussians: 2 }, port as never));
    const first = await store.loadBuffer(ply(0), {
      name: "first", priority: 2,
      attributes: [{ name: "tag", format: "u32", elementsPerGaussian: 1,
        source: { kind: "fill", value: "ones" } }],
    });
    const second = await store.loadBuffer(ply(3), { name: "second", priority: 1 });
    ...
    expect(Array.from(store.getPackedAttribute("tag")!.array as Uint32Array))
      .toEqual([0, 1]);

    const before = port.events.length;
    store.writeAttributeRange(first, "scalesOpacity", 0, 1,
      new Float32Array([2, 2, 2, 1]).buffer);
    await vi.waitFor(() => expect(port.events.slice(before)
      .some((event) => event.type === "buffers-patched")).toBe(true));
    const patch = port.events.slice(before).find((event) => event.type === "buffers-patched");
    if (patch?.type !== "buffers-patched") throw new Error("Missing patch");
    expect(patch.patches.map((item) => item.name)).toEqual(["scalesOpacity"]);
    await vi.waitFor(() => expect((store.getPackedData().scalesOpacity.array as Float32Array)[4])
      .toBe(2));
    store.dispose();
  });
  ...
  postMessage(message: WorkerInbound, transfer: ArrayBuffer[] = []): void {
    ...
    if (cloned.type === "initialize") {
      ...
      this.core.subscribe((event) => {
        ...
        const received = structuredClone({ type: "event", event } as WorkerOutbound, {
...
function ply(x: number): ArrayBuffer {
  ...
  const text = [
    ...
    "end_header", `${x} 0 0 1 1 1 10 0 0 0 1 0 0 0`,
  ].join("\n") + "\n";
